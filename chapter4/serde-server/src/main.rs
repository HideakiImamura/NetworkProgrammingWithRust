...
fn main() {
    ...
    if args[1] == "--server" {
        ...
        println!("Please provide a 3D point as three comma separeted integers");
        loop {
            ...
            stream.write_all(serde_json::to_string(&point).unwrap().as_bytes()).expect("Failed to write server");
            stream.write_all(b"\n").expect("Failed to write server");

            let mut reader = BufReader::new(&stream);
            reader.read_until(b'\n', &mut buffer).expect("Coulf not read into buffer");
